...
#### PDF (`.pdf`)
- **Provider**: `PDFProvider` (powered by Kreuzberg)
- **Capabilities**:
...
#### Images (with OCR)
- **Formats**: `.jpg`, `.png`, `.tiff`, `.bmp`, `.gif`, `.webp`, `.jp2`
- **OCR backends**: Tesseract, EasyOCR, PaddleOCR
- **Table extraction**: Detect and reconstruct tables from scanned documents
...
When running in ASGI servers (Hypercorn, Uvicorn), Kreuzberg's ProcessPoolExecutor may fail due to daemon restrictions. Our `PDFProvider` implements a subprocess workaround:
