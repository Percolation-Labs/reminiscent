...
The PostgreSQL init ConfigMap is generated by `rem cluster generate` and committed to git.
ArgoCD will deploy it as part of rem-stack. If you need to regenerate:

```bash
# Regenerate ConfigMap from SQL migrations
rem cluster generate

# Commit and push for ArgoCD to sync
git add . && git commit -m "Regenerate postgres init configmap" && git push
```
