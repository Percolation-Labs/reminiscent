...
│   ├── PDB: min 1 available
│   └── External Secrets: LLM API keys
│
└── file-processor (Deployment)
    ├── KEDA ScaledObject: 0-20 replicas (SQS-based)
...
└── CloudNativePG Cluster
    ...
    ├── Init SQL via ConfigMap (generated)
    └── Backup to S3

ConfigMaps (rem-app):
├── rem-api-config (static)
└── rem-version-info (generated by Kustomize)
```
...
│   ├── api/                     # rem-api (FastAPI + MCP)
│   ├── worker/                  # file-processor (KEDA)
│   └── postgres/                # PostgreSQL (separate namespace)
├── base/                        # Base configuration (namespace: rem-app)
│   └── kustomization.yaml      # References api + worker components
├── overlays/
...
1. Files uploaded to S3
   └─> S3 Event Notification → SQS Queue (rem-file-processing)

2. KEDA Operator (every 10 seconds)
   └─> Polls SQS via IRSA role
   └─> Gets ApproximateNumberOfMessages
   └─> Calculates: desired_replicas = ceil(messages / queueLength)
   └─> Updates HPA target

3. HorizontalPodAutoscaler
   └─> Scales Deployment based on KEDA's calculation
   └─> Respects min (0) and max (20) replica limits

4. Kubernetes Deployment
   └─> Creates/destroys pods as needed
   └─> Pods use file-processor ServiceAccount (Pod Identity/IRSA)

5. File Processor Pods
   └─> Pull messages from SQS (via IRSA role)
   └─> Download files from S3 (via IRSA role)
   └─> Process files
   └─> Delete messages from SQS
   └─> Upload results to S3

6. Karpenter (optional)
   └─> Provisions nodes if pods are pending
   └─> Deprovisions nodes when pods terminate
```
...
- 15 messages  → 2 pods (15/10 = 1.5, ceil = 2)
- 50 messages  → 5 pods
...
- Example: 1 → 2 → 4 → 8 → 16 → 20 pods in ~90s

**Scale Down (Gradual):**
- 5-minute stabilization window
- Max 50% reduction per minute
- Example: 20 → 10 → 5 → 3 → 2 → 1 → 0 over ~6 minutes
